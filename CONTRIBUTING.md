...
When it's time to ship a new version, follow these steps:

### 1. Preparation
- Update the `version` in `package.json`.
- Update `CHANGELOG.md` with the new changes.
- Ensure all tests pass and linting is clean: `pnpm run lint`.

### 2. VS Code Marketplace
We use `vsce` (Visual Studio Code Extensions) to manage releases.
```bash
# Login (if not already)
pnpx vsce login RahulDhole
...
```bash
pnpx ovsx publish -p <your-open-vsx-token>
```
...
- Push your changes and the new tag:
  ...
  ```
- Create a new Release on GitHub and attach the `.vsix` file generated by `pnpm run package`.
